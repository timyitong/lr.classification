...
public class GradientOperator implements ParameterOperator{
	...
	private double pace=1e-4;  //The Epsilon, the learning rate  1e-4
	private double threshold=5e-3; //when to converge    5e-3
	...
	public void setW(int k, int j, double val){
		w[k-1][j]=val;
	}
	private double evolveW(int k, DocVector x){ // this is the stochastic version, currently not in use
	...
	public void evolveW(DocVector x){  // this is the stochastic version, currently not in use
		for (int i=1;i<=Parameter.categories;i++)
			evolveW(i,x);
	}
	public void evolveW(ArrayList<DocVector> list,String mode){// this is the batch version
	...
	private void evolveW(int k, ArrayList<DocVector> list){// batch version for converging one catergory
		double gradient_mod=1;      // The mod of vector dl/dw to judge whether it is converged
		double old_gradient_mod=-1;  // Record the old gradient_mod to prevent extreme parameters making it impossible to converge

		double[] step=new double[Parameter.dimensions+1];  // the sum of dl/dw for each dimension
		double[] dl_dw=new double[Parameter.dimensions+1];
		...
		DocVector doc=null;
		while( gradient_mod!=old_gradient_mod && gradient_mod>threshold){ // the old and current mod comparison is to prevent some extreme situations when the parameters cannot be convergenced
			//first combine and calculate the batch
			it=list.iterator();
			int doc_index=0;

			old_gradient_mod=gradient_mod;
			gradient_mod=0;
			while (it.hasNext()){
				doc=it.next();
				double wt_x=doc.multiply(getVector(k));
				double y_minus_sigema=(doc.getY(k)-sigema(wt_x)); //y-sigema(w^t*x)
				double mod=0;
				while(doc.hasNextX()){
					double x=doc.getNextX();
					double delta=y_minus_sigema*x;
					step[doc.x_index]+=delta;
					mod+=delta*delta;
				}
				//gradient_mod+=Math.sqrt(mod);    //Gradient_v2
				doc_index++;
				doc.resetIterator();
			}
			for (int j=0;j<step.length;j++){
				double w_j=getW(k,j);
				setW(k,j,w_j+pace*(step[j]+c*w_j));
				gradient_mod+=step[j]*step[j];      //Gradient_v1: first sum vector then calculate mod
				//gradient_mod+=Math.sqrt(dl_dw[j]);    //Gradient_v2: first calculate mod of each doc then average them
			
				step[j]=0;   // after using them, make the orignial value to be 0
				dl_dw[j]=0;
			}
			gradient_mod=Math.sqrt(gradient_mod)/(list.size());   //Gradient_v1: first sum vector then calculate mod
			//gradient_mod=gradient_mod/list.size();				    //Gradient_v2: first calculate mod of each doc then average them
			System.out.println(gradient_mod);
		}
	}

	private double calculate_pr(int k, DocVector x){
		double under=1;
		
		/* This is for the single probability calculation: */
		return sigema(x.multiply(this.getVector(k)));
	}
	public void checkCategory(DocVector x){
